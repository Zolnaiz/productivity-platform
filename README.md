...
- Every zone label can carry a QR code that opens that area on a phone: its
  standard, what belongs in it, what is still red-tagged, when it was last
  audited and when the next one is due — read-only, because standing next to a
  running machine is not where a plan should be editable by accident. The link
  names the plan and the zone rather than carrying a token, so a label printed
  today still works next year.
- A red tag can be raised from the label itself. It has its own permission and
  its own narrow route rather than being a plan update: whoever finds the
  clutter may say "this does not belong here" without being able to move a
  wall, and the id, the status and the date are the server's.
...
- Work raised for somebody reaches their inbox inside the product, and — when a deployment configures one — the same words reach their email address. `MAIL_TRANSPORT=log` is the default and sends nothing, logging the recipient and the subject but never the body, because an invitation's body is a token. Invitations go out through the same transport instead of the inviter pasting a link into a chat window.
...
- Backend tests: 661 passing
- Frontend tests: 838 passing, plus eleven browser checks
