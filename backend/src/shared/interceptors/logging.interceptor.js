...
var LoggingInterceptor = function () {
    ...
    var LoggingInterceptor = _classThis = /** @class */ (function () {
        ...
        LoggingInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var now = Date.now();
            // Log request
            this.logger.log("Request: ".concat(method, " ").concat(url, " - User: ").concat((user === null || user === void 0 ? void 0 : user.id) || 'anonymous', " - Body: ").concat(JSON.stringify(body)));
            return next
                .handle()
                .pipe((0, operators_1.tap)({
                next: function (data) {
                    var response = context.switchToHttp().getResponse();
                    _this.logger.log("Response: ".concat(method, " ").concat(url, " - Status: ").concat(response.statusCode, " - Duration: ").concat(Date.now() - now, "ms"));
                },
                error: function (error) {
                    _this.logger.error("Error: ".concat(method, " ").concat(url, " - Status: ").concat(error.status || 500, " - Message: ").concat(error.message));
                },
            }));
        };
